...
public class BookingFragment extends Fragment {
    ...
    public void onViewCreated(View view, Bundle savedInstanceState) {
        ...
        int bookingIndex = getArguments() != null ? getArguments().getInt("bookingIndex", -1) : -1;

        if (bookingIndex >= 0) {
            BookingsViewModel bookingsViewModel = new ViewModelProvider(requireActivity())
                    .get(BookingsViewModel.class);

            booking = bookingsViewModel.getBookings().get(bookingIndex);
            roomPrice = booking.getRoomPrice();

            binding.checkInInput.setText(booking.getCheckInDate());
            binding.checkOutInput.setText(booking.getCheckOutDate());
            binding.nameInput.setText(booking.getFirstGuestName());
            binding.emailInput.setText(booking.getEmail());
            binding.phoneInput.setText(booking.getPhone());
